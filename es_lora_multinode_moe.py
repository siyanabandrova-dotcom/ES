            ...
            if expert_match:
                ...
                if "gate_proj" in vllm_name:
                    ...
                    if target_name in vllm_updates_dict:
                        ...
                        if vllm_name in vllm_updates_dict:
                    ...
                    if target_name in vllm_updates_dict and gate_peft_name in peft_updates_dict:
                        start = peft_updates_dict[gate_peft_name].shape[0]
                        end = min(start + weight_update.shape[0], vllm_updates_dict[target_name].shape[1])
                        weight_shard = weight_update[:end-start]
                        vllm_updates_dict[target_name][expert_idx, start:end] += weight_shard
    ...
    def generate_local_adapters(self, population_indices: list[int], es_step: int, args: Args):
        ...
        for pop_idx in population_indices:
            ...
            for layer_idx, (peft_name, weight_shape) in enumerate(self.lora_init_shapes.items()):
                ...
                # MoE expert layers naming correction for vLLM
                # PEFT: ...mlp.experts.0.gate_proj.lora_A.default.weight
                # vLLM: ...mlp.experts.0.gate_proj.lora_A.weight
                if "experts" in lora_a_name:
                    # The replace above already handled .lora_A.default.weight -> .lora_A.weight
                    # But if there are other differences in expert naming between PEFT and vLLM 
                    # we should handle them here. Standard Qwen2MoE seems consistent.
                    pass
