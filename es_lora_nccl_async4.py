...
def launch_engines(num_engines, model_name, population_size, lora_r, spread_across_nodes=False):
    ...
    engines = [
        ray.remote(num_cpus=0, num_gpus=0, scheduling_strategy=strategy)(ESNcclLLM).remote(
            ...
            worker_extension_cls="es_lora_nccl_async4.WorkerExtension",
            dtype="float16",
...
def main(args: Args):
    ...
    # For multi-node: use the head node's IP address
    if args.ray_address == "local":
        master_address = get_ip()
    else:
        # In multi-node Ray cluster, get the head node address
        # Use environment variable set by Ray or get from cluster info
        master_address = os.environ.get("RAY_HEAD_NODE_IP", get_ip())
    master_port = get_open_port()

    print(f"NCCL master address: {master_address}:{master_port}")
    ray.get([
