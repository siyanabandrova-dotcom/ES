...
def main(args: Args):
    ...
    # --- Setup/Init ---
    print("MAIN: Getting GPU count...", flush=True)
    sys.stdout.flush()
    args.num_gpus = torch.cuda.device_count()
    args.num_engines = args.num_gpus
    print(f"MAIN: Detected {args.num_gpus} GPUs", flush=True)
    sys.stdout.flush()
    ...
    for es_step in range(args.num_iterations):
        ...
        if args.verbose: print(f"Broadcasted updated weights to all engines in {broadcast_time:.4f}s ({method})")
