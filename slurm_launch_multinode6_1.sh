...
#SBATCH --nodes=1                 # <--- CHANGED: Number of nodes to use
#SBATCH --gpus-per-node=4         # <--- CHANGED: Request GPUs PER NODE
#SBATCH --time=24:00:00           # Time limit hrs:min:sec
#SBATCH --output=/home/s5e/asims.s5e/Documents/esvllm-outer/hyperscale-es-vllm/logs/geme_multinode6_1-%j.log
...
# Run with:
# sbatch $HOME/Documents/esvllm-outer/hyperscale-es-vllm/slurm_launch_multinode6_1.sh <sigma> <learning_rate> <max_tokens> <model_name> <population_size> <steps_per_adapter> <lora_r> <task> <normalize_with_std> <prompt_batch_size> <sub_dataset_size> <name_prefix>

# Example for 2 nodes with 4 GPUs each (population_size=128, 16 per GPU):
# sbatch $HOME/Documents/esvllm-outer/hyperscale-es-vllm/slurm_launch_multinode6_1.sh 0.001 0.001 4096 "Qwen/Qwen3-4B" 1024 4 1 "math2:deepscaler40k" "normalize-with-std" 16 "null" "multinode-test6_1"
# sbatch $HOME/Documents/esvllm-outer/hyperscale-es-vllm/slurm_launch_multinode6_1.sh 0.001 0.001 1024 "Qwen/Qwen3-0.6B" 128 4 1 "math2:deepscaler40k" "normalize-with-std" 16 "null" "debug-multinode-test6_1"
# sbatch $HOME/Documents/esvllm-outer/hyperscale-es-vllm/slurm_launch_multinode6_1.sh 0.001 0.001 32 "Qwen/Qwen3-0.6B" 32 4 1 "zeros" "normalize-with-std" 16 "null" "debug-multinode-test6_1"
# sbatch $HOME/Documents/esvllm-outer/hyperscale-es-vllm/slurm_launch_multinode6_1.sh 0.001 0.001 4096 "Qwen/Qwen3-1.7B" 1024 4 1 "math2:deepscaler40k" "normalize-with-std" 16 "null" "big6_1"
...
echo "Python script output will appear below:"
echo "---------------------------------"

# Note: The python script connects to the Ray cluster we just built
# Adding timeout to prevent indefinite hangs (3 hours = 10800 seconds)
timeout 10800 python -u es_lora_nccl_async6.py \
    --sigma $sigma \
